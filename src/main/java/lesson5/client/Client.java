...
public class Client {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("Ïîäêëþ÷èëèñü ê ñåðâåðó: tcp://localhost: " + Server.PORT);
            Scanner serverIn = new Scanner(serverSocket.getInputStream());
            String input = serverIn.nextLine();
            System.out.println("Ñîîáùåíèå îò ñåðâåðà: " + input);
            ...
            throw new RuntimeException("Íå óäàëîñü ïîäêëþ÷èòü ê ñåðâåðó: " + e.getMessage(), e);
        }
    }
}
        ...
        try (PrintWriter out = new PrintWriter(serverSocket.getOutputStream(), true)) {
            while (true) {
                ...
                if (Objects.equals("exit", msgFromConsole)) {
                    System.out.println("Îòêëþ÷àåìñÿ...");
                    break;
                }
            }
        } catch (IOException e) {
            System.err.println("Îøèáêà ïðè îòïðàâêå íà ñåðâåð: " + e.getMessage());
        }

        try {
            serverSocket.close();
        } catch (IOException e) {
            System.err.println("Îùèáêà ïðè îòêëþ÷åíèè îò ñåðâåðà " + e.getMessage());
        }
    }
}
    ...
    public void run() {
        try (Scanner scannerIn = new Scanner(serverSocket.getInputStream())) {
            while (scannerIn.hasNext()) {
                ...
                System.out.println("Ñîîáùåíèå îò ñåðâåðà: " + inputFromServer);
            }
        } catch (IOException e) {
            System.out.println("Îøèáêà ïðè ÷òåíèè ñ ñåðâåðà: " + e.getMessage());
        }

        try {
            serverSocket.close();
        } catch (IOException e) {
            System.err.println("Îøèáêà ïðè îòêëþ÷åíèè îò ñåðâåðà " + e.getMessage());
        }
